...
bugslist <- fread("../hiv.t32.concat.humann.long/all_bugs_list.tsv") %>% as.data.frame()
...
multi <- plot_grid(plot_1, GP_legend,  rel_widths = c(1, .25))
multi
ggsave("../figures/taxa-bar-Risk-phyum-genus-microshades.pdf",plot=multi,height=6,width=10)
...
prev.stats$expected_Prevotella <- metadata$`%Prevotella`
prev.stats$Risk_Status <- metadata$Risk_Status

metadata[prev.stats$expected_Prevotella < 0.05, ]
metadata[prev.stats$Prevotella < 5, ]
