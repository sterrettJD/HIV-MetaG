...
### Taxa bar plot of lower than expected Prevotella
```{r, warning=T, fig.height=9, fig.width=8}
# prep the microshades colors
...
firm.only.weird.samps.otu[order(rowSums(firm.only.weird.samps.otu), decreasing=T),] %>% as.data.frame() %>%
    select("DM087", "LG026", "LG043", "LG045", "LG049") %>%
    head(n=15) %>% 
    knitr::kable()
