...
tree.bins.info[grepl(x=tree.bins.info$Subset.Name, pattern="GGB"), 
               "Subset.Name"] <- NA
...
p %<+% tree.bins.info + 
    geom_label(aes(label = Subset.Name), nudge_x=0.1)

p <- ggtree(tree, layout="ellipse") + 
    theme_tree()

p %<+% tree.bins.info + 
    geom_label(aes(label = Subset.Name), nudge_y=-0.2) + 
    geom_point(aes(shape=Found, color=Found#, color=Known
                   ),
               size=5, na.rm=T)
```
